...
<div style="padding:5px; line-height: 1.6;">
    ...
    <div style="font-family:Century Gothic;font-size:19px">
        To,<br>
        <p style="font-family:Century Gothic;">Mr/Ms. {{$model->emp_name}},</p>
        <p style="font-family:Century Gothic;">{{$model->emp_code}},</p>
        <p style="font-family:Century Gothic;">{{$model->designation->designation_name}}.</p>
        <p style="font-family:Century Gothic;"><b>Dear Mr/Ms. {{$model->emp_name}}</b></p>
        <p style="font-family:Century Gothic;">Congratulation!!</p>
        <p>This has reference to your application and the subsequent interview you had with us, we are pleased to offer
            you employment as <strong>“{{$model->designation->designation_name}}”</strong> on contractual basis for
            project <strong>{{ $model->project->project_name}} {{ $model->locantions->location}} </strong>for client
            <strong>L&T Constructions, Smart World And Communications</strong> for a specific period of 12 (Twelve)
            months from the
            <strong>{{ ($model->date_of_joining ? date('d-m-Y', strtotime($model->date_of_joining)) : '')}}</strong>,
